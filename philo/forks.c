...
/*   Updated: 2026/08/30 16:12:43 by lrouchon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	lock_fork(t_philosopher *philosopher, t_fork *fork)
{
	...
	print_lock(philosopher, "has taken a fork", COLOR_YELLOW);
	return (1);
}
	...
	if (philosopher->index % 2 == 0)
	{
		...
		if (!lock_fork(philosopher, philosopher->fork))
			return (pthread_mutex_unlock(
					&philosopher->previous->fork->fork_mutex), 0);
	}
	else
	{
		if (!lock_fork(philosopher, philosopher->fork))
			return (0);
		if (!lock_fork(philosopher, philosopher->previous->fork))
			return (pthread_mutex_unlock(&philosopher->fork->fork_mutex), 0);
	}
	return (1);
}
